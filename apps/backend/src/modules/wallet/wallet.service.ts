...
// ─────────────────────────────────────────────────────────────────────────────
export async function deductTripCredit(
  agencyId: string,
  tripId: string,
): Promise<{ success: boolean; tripsRemainingAfter: number; lowTrips: boolean }> {
  ...
  const [updated] = await db
    ...
    .set({
      trips_remaining:        sql`trips_remaining - 1`,
      trips_used_this_month:  sql`trips_used_this_month + 1`,
      updated_at:             new Date(),
    })
    .where(eq(agencyWallets.agency_id, agencyId))
    .returning({
  ...
  if (lowTrips) {
    try {
      await emitSocketEvent(`agency:${agencyId}`, 'low_trips_alert', {
        agencyId,
...
// ─────────────────────────────────────────────────────────────────────────────
export async function topUpTrips(
  agencyId: string,
  tripsToAdd: number,
  description?: string,
  referenceId?: string,
): Promise<{ tripsRemainingAfter: number }> {
  if (tripsToAdd <= 0) throw Object.assign(new Error('Top-up amount must be a positive integer'), { statusCode: 400 });
  ...
  const [updated] = await db
    .update(agencyWallets)
    .set({
      trips_remaining: sql`trips_remaining + ${tripsToAdd}`,
      updated_at: new Date(),
    })
    .where(eq(agencyWallets.agency_id, agencyId))
    .returning({ trips_remaining: agencyWallets.trips_remaining });
...
export async function updateWalletConfig(
  agencyId: string,
  updates: { low_trip_threshold?: number },
): Promise<typeof agencyWallets.$inferSelect> {
  await getOrCreateWallet(agencyId);
