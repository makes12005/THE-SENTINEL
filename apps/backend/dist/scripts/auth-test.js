...
function pass(id, name, notes = '') {
    results.push({ id, name, status: '✅', notes });
    log(`  ✅ PASS: ${name}${notes ? ` — ${notes}` : ''}\n`);
}
function fail(id, name, notes) {
    results.push({ id, name, status: '❌', notes });
    log(`  ❌ FAIL: ${name} — ${notes}\n`);
}
function warn(id, name, notes) {
...
async function test10() {
    ...
    `;
      ...
      `;
            ...
            deleted.forEach((r) => log(`     - ${r.email} (${r.id})\n`));
...
function buildReport() {
    ...
    const table = results.map(r => `| ${String(r.id).padEnd(5)} | ${r.name.padEnd(26)} | ${r.status}   | ${r.notes.slice(0, 65)} |`).join('\n');
...
async function main() {
    ...
    if (fs.existsSync(envPath)) {
        ...
        for (const line of lines) {
            ...
            if (key && rest.length) {
                (_a = process.env)[_b = key.trim()] ?? (_a[_b] = rest.join('=').trim().replace(/^["']|["']$/g, ''));
            }
        }
    }
    log('╔══════════════════════════════════════════════════════╗\n');
    ...
    for (let attempt = 1; attempt <= 15; attempt++) {
        try {
            ...
            if (r.ok) {
                ready = true;
                log(`  ✅ Backend ready (attempt ${attempt})\n`);
                break;
            }
        }
        catch { /* retry */ }
