...
function getOAuth2Client() {
    return new google_auth_library_1.OAuth2Client(getEnv('GOOGLE_CLIENT_ID'), getEnv('GOOGLE_CLIENT_SECRET'), getEnv('GOOGLE_CALLBACK_URL'));
}
async function issueTokens(user) {
    const jwtSecret = getEnv('JWT_SECRET');
    const refreshSecret = getEnv('REFRESH_TOKEN_SECRET');
    const accessToken = jsonwebtoken_1.default.sign({ id: user.id, role: user.role, agencyId: user.agency_id, name: user.name }, jwtSecret, { expiresIn: '15m', subject: user.id });
    const refreshToken = jsonwebtoken_1.default.sign({ role: user.role, agencyId: user.agency_id, type: 'refresh' }, refreshSecret, { expiresIn: '30d', subject: user.id });
...
// ─── Route plugin ─────────────────────────────────────────────────────────────
const googleAuthRoutes = async (fastify) => {
    /**
    ...
    fastify.get('/google/callback', async (request, reply) => {
        ...
        const errorRedirect = (msg) => reply.redirect(`${webAppUrl}/login?error=${encodeURIComponent(msg)}`);
        // ── Error from Google
        ...
        if (!user) {
            ...
            const dummyHash = await bcryptjs_1.default.hash((0, crypto_1.randomBytes)(32).toString('hex'), 12);
