...
async function run() {
  ...
  const t1 = await req('POST', '/api/routes', {
    token,
    body: { name: 'Ahmedabad to Surat', fromCity: 'Ahmedabad', toCity: 'Surat' },
  });
  ...
  const t2 = await req('POST', '/api/routes', {
    token,
    body: { name: 'Surat to Mumbai', from_city: 'Surat', to_city: 'Mumbai' },
  });
  ...
  // Full trip flow: stops + trip + upload
  let flowPass = false;
...
Priya Patel,+919876540002,Vadodara`;
  ...
  console.log(JSON.stringify({ generatedAt: new Date().toISOString(), results, ids: { routeCamel, routeSnake, busId, conductorId, driverId, tripId } }, null, 2));
}
